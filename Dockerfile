# Based on https://github.com/zmartzone/mod_auth_openidc/blob/master/Dockerfile-alpine
FROM alpine:3.10

ENV MOD_AUTH_OPENIDC_REPOSITORY https://github.com/zmartzone/mod_auth_openidc.git

ENV MOD_AUTH_OPENIDC_BRANCH fapi
...
ADD *.template /etc/apache2/conf.d/
ADD entrypoint.sh /usr/local/sbin/
ADD *.json /var/www/localhost/htdocs/
ADD server.pem /etc/x509/https/tls.crt
ADD server-key.pem /etc/x509/https/tls.key
ADD ca.pem /usr/local/share/ca-certiicates/ca.pem
